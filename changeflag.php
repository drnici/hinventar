	...
	if ( !empty ( $res_data["res_id"] ) )
	{
		...
		if ( in_array ( $value , $allowed_fields ) )
		{
			...
			if ( $value == "res_visum_bb" || $value == "res_visum_lde" || $value == "res_visum_gv" )
			{
				...
				{
					// ADMIN (f�r BB und GV) oder Lernende (bei Visum Lde)
					header ( "Location: ./?person_s_semester=" . $person_s_semester . "&periode_id=" . $periode_id . "&" );
					die ( );
				}
			...
			// Text f�r History-Eintrag unterscheiden
			if ( $target_value == 1 )
			{
			...
			{
				if ( $value == "res_release" ) 			$history_text = "Qualifikation zur�ckgezogen";
				if ( $value == "res_discuss" ) 			$history_text = "Qualifikation nicht besprochen";
				if ( $value == "res_visum_bb" ) 		$history_text = "Visum Berufsbildner/in zur�ckgezogen";
				if ( $value == "res_visum_lde" ) 		$history_text = "Visum Lernende/r zur�ckgezogen";
				if ( $value == "res_visum_gv" ) 		$history_text = "Visum gesetzliche Vertretung zur�ckgezogen";
			}
