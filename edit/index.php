...
if ( isset ( $res_id ) AND !empty ( $res_data["res_id"] ) AND !$res_data["res_release"] )
{
	...
	// Prüfen, ob alle Punkte ausgewählt wurden -> d.h. ob die Quali vollständig ist
	...
	// Ist die Quali vollständig?
	$quali_complete = true;
	if ( $antwort_done < $antwort_undone )
	{
		$quali_complete = false;
		echo ( "<p class=\"warning\"><b>Achtung:</b> Die Qualifikation wurde noch nicht vollständig ausgefüllt (" . $antwort_done . " von " . $antwort_undone . " Fragen bewertet).</p>\n" );
	}
	?>
    ...
    if ( $res_data["periode_id"] == 1 )
    {
        ...
        <p><input type="radio" name="res_probezeit" value="1" class="width_auto" <?PHP if ( $res_data["res_probezeit"] == 1 ) echo ( " checked=\"checked\"" ); ?> /> Der/Die Lernende erfüllt die Voraussetzungen in der Praxis für eine definitive Übernahme in die Lehrzeit.</p>
        
        <p><input type="radio" name="res_probezeit" value="2" class="width_auto" <?PHP if ( $res_data["res_probezeit"] == 2 ) echo ( " checked=\"checked\"" ); ?> /> Der/Die Lernende erfüllt die Voraussetzungen in der Praxis für eine definitive Übernahme in die Lehrzeit nur teilweise. Wir schlagen vor, die Probezeit um 3 Monate zu verlängern.</p>
                
        <p><input type="radio" name="res_probezeit" value="3" class="width_auto" <?PHP if ( $res_data["res_probezeit"] == 3 ) echo ( " checked=\"checked\"" ); ?> /> Der/Die Lernende erfüllt die Voraussetzungen in der Praxis für eine definitive Übernahme in die Lehrzeit nicht. Wir beantragen, das Lehrverhältnis aufzulösen.</p>
    	<?PHP
		$j++;
    }
    ?>	
    ...
    <p>
	<input type="submit" name="btn" value="Änderungen speichern" class="btn" />
